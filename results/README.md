...
accuracy rises from 4/20 to 16/20 over sixteen cycles and peaks at 19/20 at
...
accuracy fluctuates between 7 and 19 without collapsing. One fact is never
learned. We conclude that generate-judge-restore selection makes self-training
viable at this scale, and that the limiting factor is the diversity of the
model's own samples rather than the capacity of the update.
...
| Loops (no answer) | 3 | 2 | 4 | 0 | 10 | 1 | 13 |
...
other items are trained. Of 209 candidates over 32 cycles, 53 are kept. Most
...
Scores fluctuate between 7 and 19, loops rise to as many as 13 of 20, and
items that were 4/4 for many cycles lose and regain prompts with no update
between cycles.
...
**Unlearnable item.** Turkey is never fixed on all four prompts in 35 attempts
during the lift phase. Its candidates train to near-zero answer loss under
their own reasoning, and free generation still concludes Istanbul: the
correction holds only conditioned on the sampled reasoning, which the model
does not reproduce when it thinks unprompted.
...
sampling prompt contains the answer. A second run of the same loop in PyTorch
on a Colab T4, in fp16, is in progress and will be added as a replicate.
