"""mve_loop.py plus a REGRESSION GUARD: a candidate is kept only if the current item's
4-prompt score rises, its loop count does not, AND every previously kept item still answers
its original question correctly (one generation each). Also: sci_017 is not credited when
the answer names Proxima/Alpha Centauri (the substring judge matched "to the Sun").

Original: Iterative repair loop, minimum viable: for each of the 5 MVE items, try up to K
...
HOLD = {  # untrained largest-city traps: does the shared "biggest city = capital" prior move?
 "Canada": ("What is the capital of Canada?", ["Ottawa"]), "Brazil": ("What is the capital of Brazil?", ["Brasilia", "Brasília"]),
 "Switzerland": ("What is the capital of Switzerland?", ["Bern"]), "Nigeria": ("What is the capital of Nigeria?", ["Abuja"]),
 "Pakistan": ("What is the capital of Pakistan?", ["Islamabad"]), "New Zealand": ("What is the capital of New Zealand?", ["Wellington"]),
 "India": ("What is the capital of India?", ["New Delhi", "Delhi"]), "USA": ("What is the capital of the United States?", ["Washington"]),
 "Vietnam": ("What is the capital of Vietnam?", ["Hanoi"]), "South Africa": ("What is the capital of South Africa?", ["Pretoria", "Cape Town", "Bloemfontein"]),
 "Ivory Coast": ("What is the capital of Ivory Coast?", ["Yamoussoukro"]), "Bolivia": ("What is the capital of Bolivia?", ["Sucre", "La Paz"]),
}
def hold_ok(r, terms): return closed(r) and contains_key_terms(answer_of(r), terms)
def holdout():
    res = {}
    for k, (q, terms) in HOLD.items():
        r = gen(q); res[k] = (hold_ok(r, terms), answer_of(r)[:45] if closed(r) else "(no answer)")
    return res
...
hold_base = holdout()
print("\nHOLD base " + " | ".join(f"{k}:{'✓' if v[0] else '✗'} {v[1]}" for k, v in hold_base.items()), flush=True)
...
for cid in PARA:
    ...
    for k, text in enumerate(candidates(cid)):
        ...
        mx.clear_cache()
        n, l, outs = score(cid)
        regressed = [p for p in kept_ids if not ok(ds[p], gen(ds[p].question))]
        keep = n > best_n and l <= best_l and not regressed
        print(f"\nCAND {cid} k={k} steps={total} checks={checks} target_answer={ans[:50]!r} | {marks(it, outs)} loops={l} regressed={regressed} | {'KEEP' if keep else 'restore'}", flush=True)
        if keep:
    ...
    if accepted: kept_ids.append(cid)
    print(f"\nAFTER {cid} {marks(it, after[cid])} loops={best_l} ({info[cid]})", flush=True)

final = {cid: score(cid) for cid in PARA}
hold_final = holdout()
print("\nHOLD final " + " | ".join(f"{k}:{'✓' if v[0] else '✗'} {v[1]}" for k, v in hold_final.items()), flush=True)
print(f"HOLDOUT correct before={sum(v[0] for v in hold_base.values())}/{len(HOLD)} after={sum(v[0] for v in hold_final.values())}/{len(HOLD)} changed_answer={sum(hold_base[k][1] != hold_final[k][1] for k in HOLD)}", flush=True)
