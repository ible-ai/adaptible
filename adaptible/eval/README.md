...
| `rationale` (default) | `{rationale}\n</think>\n\n{revision}{eos}` | all of it | The rationale is reasoning that concludes the revision: for `self_generated` the revision generation's own think block (`split_think(revision_raw)`); for `ground_truth` one extra generation with `harness.make_rationale_prompt` (`"{question}\n\nThe correct answer is: {answer}\nReason it through step by step, then state the answer."`), reasoning taken with `revise.rationale_from_output`: the think block, or the *whole* output when the model never emitted `</think>` (it generates inside the open think block, so a `</think>`-less output is reasoning that ran to the cap, not an answer), capped at `rationale_max_tokens` (`--rationale_max_tokens`; default 512) at the last sentence boundary (`ItemResult.rationale_tokens` / `rationale_truncated`). An item with no rationale at all is **skipped, not trained** (`ItemResult.rationale_missing`, counted in `EvaluationResult.rationale_missing_count` / `SeedTrajectory.rationale_missing_count`, excluded from train and holdout metrics like an invalid revision); `make_revision_training_example` raises `ValueError("rationale required")` rather than falling back. The fallback used to be the `empty` target, and a 40-item screen in which 13/32 items took it collapsed the model exactly as the `empty` run did. `ItemResult.rationale_text` and the report card carry the rationale used. |
...
Per item, `ItemResult` records `train_steps`, `train_initial_loss`, `train_final_loss` (answer loss), `train_final_train_loss` (whole-target loss), `train_hit_cap`, and (with rehearsal) `train_rehearsal_initial_loss`, `train_rehearsal_final_loss`, and `train_rehearsal_active_steps`; the `training_events.training_iterations` column stores the steps the correction actually took. The verbose log prints `Trained (3 steps, loss 6.05 → 0.58 (train 0.80, rehearsal 0.38→0.36 (active 3/9)), 4.1s)` per item and the summary (and report header) carries `Training: mean 3.2 steps/item (cap 12, answer-loss target 0.60), mean final answer loss 0.55 (train loss 0.80, rehearsal 0.41); N items hit the cap; R rationales missing; rehearsal active in P% of steps`; the `train loss` part appears only when some item reported a whole-target loss and the `rehearsal` parts only when some item trained with rehearsal. Only the correction's answer loss is ever compared against the target. The server's `/trigger_review` path (`self_correct_and_train`) uses `StatefulLLM(loss_target=0.6, max_train_steps=12)`.

The `empty` run above also drifted unrelated facts ("The skin" for the largest planet). `rehearsal_k` (`EvaluationConfig.rehearsal_k`, `MetaLearningConfig.rehearsal_k`, `--rehearsal_k`; default 0) folds `k` self-distillation examples into every step of a correction: other *trained-split* items whose baseline answer was judged correct, with the model's own full baseline output (`{think}</think>\n\n{answer}{eos}`, whole target in the loss) as the target. The item is one `StatefulLLM.train_on_examples(correction, rehearsal, loss_target=..., max_steps=training_iterations, rehearsal_weight=...)` call. Each step computes the correction's gradient and every rehearsal example's gradient as separate single-sequence passes (never a padded batch; a `(1+k, L)` batch of rehearsal targets that ran to the generation cap exhausted a 16 GB machine), combines them as `grad(correction) + rehearsal_weight * mean(grad(rehearsal))` (`llm.combine_grads`), and applies one optimizer update; training stops when the *correction* loss is below `loss_target` or at the step cap. `rehearsal_weight` (`EvaluationConfig.rehearsal_weight`, `MetaLearningConfig.rehearsal_weight`, `--rehearsal_weight`; default 1.0) scales the rehearsal term; `0` keeps the passes and the loss reporting but lets only the correction move the weights. With `rehearsal_k=0`, or an empty pool, the item is a plain `train_on_example` call.

Rehearsal is a hinge, not an objective (`rehearsal_margin`: `EvaluationConfig.rehearsal_margin`, `MetaLearningConfig.rehearsal_margin`, `--rehearsal_margin`; default 0.05). Each rehearsal example's loss at the call's first step is its anchor, and on every step only the examples whose loss is more than `rehearsal_margin` above their anchor contribute a gradient (`llm.active_rehearsal`; the mean in `combine_grads` is over that active set, and with none active the step is the correction gradient alone). The first step never applies a rehearsal gradient. With the gradient always on, a 40-item screen (k=4, ~12 pool items, ~860 rehearsal gradient steps) drove the rehearsal loss from ~0.38 to 0.09 and the model memorised its own dozen outputs; the run still collapsed (responses 718 → 32 tokens, 36/40 empty-think, holdout 1/8 → 0/8). `TrainingStats.rehearsal_initial_loss` / `rehearsal_active_steps` and the per-item / summary figures above say how often the hinge fired; whether the hinge changes the outcome has not been measured yet.
...
from adaptible.eval.harness import lora_model_kwargs  # not re-exported from adaptible.eval yet
...
All results are also written to `<outputs>/adaptible.db` (see `adaptible/db.py`), where responses are stored raw and re-judged at query time.
