...
from .. import llm
from ..llm import StatefulLLM, TrainingStats
from ..db import (
    Database,
    Example,
    Experiment,
...
DEFAULT_LORA_RANK: int = int(llm._LORA_PARAMETERS["rank"])
DEFAULT_LORA_LAYERS: int = int(llm._NUM_LORA_LAYERS)
DEFAULT_LORA_SCALE: float = float(llm._LORA_PARAMETERS["scale"])
...
class EvaluationConfig:
    ...
    Attributes:
        ...
        rehearsal_margin: The rehearsal hinge (``llm.active_rehearsal``): a
...
def _train_one_item(
    ...
    ``rehearsal_margin`` is the rehearsal hinge (``llm.active_rehearsal``)
