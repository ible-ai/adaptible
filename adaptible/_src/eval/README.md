...
Rehearsal is a hinge, not an objective (`rehearsal_margin`: `EvaluationConfig.rehearsal_margin`, `MetaLearningConfig.rehearsal_margin`, `--rehearsal_margin`; default 0.05). Each rehearsal example's loss at the call's first step is its anchor, and on every step only the examples whose loss is more than `rehearsal_margin` above their anchor contribute a gradient (`_llm.active_rehearsal`; the mean in `combine_grads` is over that active set, and with none active the step is the correction gradient alone). The first step never applies a rehearsal gradient. With the gradient always on, a 40-item screen (k=4, ~12 pool items, ~860 rehearsal gradient steps) drove the rehearsal loss from ~0.38 to 0.09 and the model memorised its own dozen outputs; the run still collapsed (responses 718 → 32 tokens, 36/40 empty-think, holdout 1/8 → 0/8). `TrainingStats.rehearsal_initial_loss` / `rehearsal_active_steps` and the per-item / summary figures above say how often the hinge fired; whether the hinge changes the outcome has not been measured yet.
