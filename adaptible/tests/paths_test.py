...
from adaptible._src import _paths
from adaptible._src.db import Database


class OutputsDirTest(unittest.TestCase):
    def test_env_override_wins(self):
        with mock.patch.dict(os.environ, {"ADAPTIBLE_OUTPUTS_DIR": "/some/where"}):
            self.assertEqual(_paths.outputs_dir(), Path("/some/where"))
            self.assertEqual(_paths.default_db_path(), Path("/some/where/adaptible.db"))
            self.assertEqual(
                _paths.default_checkpoint_path(),
                Path("/some/where/autonomous/checkpoint"),
            )
            self.assertEqual(
                _paths.autonomous_state_path(),
                Path("/some/where/autonomous/state.json"),
            )
            self.assertEqual(
                _paths.autonomous_log_dir(), Path("/some/where/autonomous/logs")
            )

    def test_defaults_to_cwd_outputs(self):
        with mock.patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ADAPTIBLE_OUTPUTS_DIR", None)
            self.assertEqual(_paths.outputs_dir(), Path.cwd() / "outputs")

    def test_never_resolves_into_package_dir(self):
        with mock.patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ADAPTIBLE_OUTPUTS_DIR", None)
            package_dir = Path(_paths.__file__).parent
            self.assertNotIn(package_dir, _paths.default_db_path().parents)
