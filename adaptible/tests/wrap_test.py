...
from adaptible._src.wrap.repair import Controller, answer
...
class WrapperTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_retirement_failure_does_not_undo_an_accepted_adapter(self):
        with self.assertLogs("adaptible._src.wrap.repair", level="WARNING"):
    ...
    async def test_import_wrapper_does_not_load_mlx_or_torch(self):
        ...
        result = subprocess.run(
            [
                sys.executable,
                "-c",
                "import sys; import adaptible._src.wrap.app; assert 'mlx' not in sys.modules; assert 'torch' not in sys.modules",
            ],
            capture_output=True,
            text=True,
        )
    ...
    async def test_ollama_unloads_only_selected_base_and_active_adapter(self):
        ...
        with (
            ...
            try:
                ...
                with mock.patch("adaptible._src.wrap.runtime.run_command", new=remove):
                    await runtime.discard(candidate)
...
class TrainerPrecisionTest(unittest.TestCase):
    ...
    def test_an_f32_checkpoint_trains_in_f32(self):
        ...
        from adaptible._src.wrap.train import checkpoint_dtype
