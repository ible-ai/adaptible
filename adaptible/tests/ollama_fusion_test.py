...
class OllamaFusionTest(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.patch = patch(
            "adaptible._src.wrap.runtime.run_command", side_effect=self.command
        )
        self.patch.start()
        self.addCleanup(self.patch.stop)

    async def new_runtime(self):
        with patch("adaptible._src.wrap.runtime.shutil.which", return_value="ollama"):
    ...
    async def command(self, *args, **kwargs):
        ...
        if args[1:3] == ("-m", "adaptible._src.wrap.gguf_fusion"):
            if self.fail_fusion:
