...
from adaptible._src.wrap.runtime import LlamaCpp, run_command
...
class RuntimeCommandTest(unittest.IsolatedAsyncioTestCase):
    async def test_cancel_during_creation_reaps_child_after_handle_arrives(self):
        ...
        with patch(
            "adaptible._src.wrap.runtime.asyncio.create_subprocess_exec", delayed_spawn
        ):
    ...
    async def test_cancel_force_kills_child_that_ignores_termination(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch(
                    "adaptible._src.wrap.runtime.asyncio.create_subprocess_exec",
                    capture_spawn,
                ),
                patch("adaptible._src.wrap.runtime._COMMAND_SHUTDOWN_TIMEOUT", 0.05),
            ):
                task = asyncio.create_task(
...
class LlamaCppPrecisionTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_every_launch_pins_all_three(self):
        from adaptible._src.wrap.runtime import LLAMA_CPP_FULL_PRECISION
