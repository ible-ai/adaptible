...
from adaptible.wrap.__main__ import parser as wrapper_parser, serve
...
class WrapperDemoContractTest(unittest.TestCase):
    ...
    def test_server_bounds_graceful_shutdown_before_runtime_cleanup(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with (
                mock.patch(
                    "adaptible.wrap.__main__.LlamaCpp", return_value=runtime
                ),
                mock.patch(
                    "adaptible.wrap.__main__.read_architecture",
                    return_value="qwen2",
                ),
                mock.patch(
                    "adaptible.wrap.__main__.repairable",
                    return_value=(True, ""),
                ),
                mock.patch(
                    "adaptible.wrap.__main__.fingerprint_base",
