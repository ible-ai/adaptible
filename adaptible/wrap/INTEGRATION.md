...
The included terminal client (`python -m adaptible.local.cli`) supports `/down`,
...
```sh
python -m unittest adaptible.wrap.wrap_test adaptible.wrap.cli_test \
  adaptible.wrap.lmstudio_test adaptible.wrap.vllm_test -v
python -m unittest adaptible.wrap.model_source_test -v
python -m unittest adaptible.wrap.retention_test scripts.wrapper_recurrent_test -v
```
...
ADAPTIBLE_VLLM_EXECUTABLE=/path/to/vllm VLLM_CPU_KVCACHE_SPACE=1 \
  python -m unittest adaptible.wrap.vllm_test.VLLMLiveAdapterTest -v
```
...
ADAPTIBLE_TEST_OLLAMA_MODEL=qwen2.5:0.5b \
  python -m unittest scripts.wrapper_integration_test -v
...
ADAPTIBLE_TEST_LLAMA_SERVER=/path/to/llama-server \
python -m unittest scripts.wrapper_recurrent_test.RecurrentLiveTest.test_llama_cpp -v
```
